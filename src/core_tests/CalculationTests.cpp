...
TEST_F(CalculationTest, OrderValidation)
{
    ...
    // Валидация должна вернуть ошибку, так как не заполнен обязательный параметр
    auto result = service_->ValidateOrder(order.id);
    
    // Заказ без обязательных параметров не должен пройти валидацию
    EXPECT_FALSE(result.isValid) << "Заказ без обязательных параметров должен быть невалидным";
    if (!result.isValid)
    {
        EXPECT_FALSE(result.errorMessage.empty()) << "Должно быть сообщение об ошибке";
    }
}
